...
def pdftotext(my_pdf, textfile_name, table, left, top, width, height, table_number):
    ...
    subprocess.call(["pdftotext", "-f", str(table), "-l", str(table), "-x", str(int(left)), "-y", str(int(top)), "-W",
                     str(int(width)), "-H", str(int(height)), "-layout", my_pdf,output])
...
def one_table(my_pdf, page_no, num_lines, top_coords, line_coords, line_count, table_header_count, table_count,
              textfile_name, print_mode=False):
    ...
    pdftotext(my_pdf, textfile_name, page_no+1, left, top, width, height, table_count)
...
def multiple_tables(my_pdf, page_no, line_count, top_coords, line_coords, table_header_count, table_count, textfile_name
                    , print_mode=False):
    bottom, top, left, right = find_table_coords(top_coords, line_coords, line_count, table_header_count)
    width = right-left
    height = bottom-top
...
def function_calls(kernel_size=5, rho=1, theta=np.pi / 2, threshold=50, minLineLength=500, maxLineGap=1,
                   dst='output/txt/', file='temp', print_mode=False):
    table_count = 0
