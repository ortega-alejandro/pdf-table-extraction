...
def tags_by_date(TAGS, ARTICLE_TAG, DATE_TAG, LAST_SCRAPE_DATE, SORTED_SITE, DATE_FORM, links, html_soup, pages):
    for tag_type,tag in TAGS:
        ...
        for each in links_page:  
            ...
            #Note: each date is associated with a list of articles
            articles = each.find_all(ARTICLE_TAG[0],class_=ARTICLE_TAG[1])
            ...
            if date is not None:
                ...
                if date>=last_date:
                    #combine 2 lists instead of just appending just one item
                    links+=articles
                elif SORTED_SITE:
                    pages=''
                    break
...
def scrape_page(current, links, prefix_length, URL_PREFIX, pdf_links, xlsx_links, links_visited, all_links):
    for each in links:
        ...
        for link in anchor_tags:
            if "Download All" not in link.text:
                #Exclude the link to "download all" at one time
                pdf_url= link['href']
...
def multiple_pages(NEXT_TEXT, prefix_length, URL_PREFIX, TAGS, ARTICLE_TAG, DATE_TAG, DATE_FORM, LAST_SCRAPE_DATE, SORTED_SITE, all_links, pdf_links, xlsx_links, links_visited, PRINT_MODE):
    page_num = 1
    while all_links:
        ...
        while all_links:
            ...
            links, pages = tags_by_date(TAGS, ARTICLE_TAG, DATE_TAG, LAST_SCRAPE_DATE, SORTED_SITE, DATE_FORM, [], html_soup, pages)
...
def scrape_website(NEXT_TEXT, TAGS, ARTICLE_TAG, DATE_TAG, DATE_FORM, LAST_SCRAPE_DATE, SORTED_SITE, pdf_links = [], xlsx_links = [], links_visited = [], PRINT_MODE=True):
    
    URL_PREFIX = re.search('.*org/|.*com/|.*edu/',URL).group(0)
    prefix_length = len(URL_PREFIX)
    all_links = [URL]

    pdf_links, xlsx_links = multiple_pages(NEXT_TEXT, prefix_length, URL_PREFIX, TAGS, ARTICLE_TAG, DATE_TAG, DATE_FORM, LAST_SCRAPE_DATE, SORTED_SITE, all_links, pdf_links, xlsx_links, links_visited, PRINT_MODE)
...
NEXT_TEXT = 'next ›'
URL = 'https://data.adb.org/search/type/dataset?sort_by=changed&page=0%2C0&q=search/type/dataset'
TAGS = [('h2','node-title'),('div','content')]
ARTICLE_TAG = ('div','item-list')
LAST_SCRAPE_DATE = '2018-07-17'
SORTED_SITE = True
DATE_TAG = ('div','field-name-field-modified-date')
DATE_FORM = '%Y-%m-%d'

pdf_links, xlsx_links = scrape_website(NEXT_TEXT, TAGS, ARTICLE_TAG, DATE_TAG, DATE_FORM, LAST_SCRAPE_DATE, SORTED_SITE) 
